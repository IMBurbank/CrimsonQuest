...
class OverlayInventory extends React.Component {
  ...
  optKeyDown(e) {
    ...
    else if ((el === 'Space' || el === 'Enter' || el == 'Numpad Enter') && getById(this.state.bRowId + this.state.row)) {
  ...
  setList(inv, header) {
    ...
    for (props in inv) {
      if (inv[props].type === header && inv[props].count > 0) {
        ...
        if (el.type === 'consumable') list.push([el.count, el.name, stats, el.buy]);
        else equipped = el.equipped ? 'E' : ' ', list.push([equipped, el.name, stats, el.buy]);
      }
    }

    list.sort( (a, b) => {return a[3] !== b[3] ? b[3] - a[3] : a[1] < b[1] ? -1 : 1;});
