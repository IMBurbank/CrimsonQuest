...
const calcRenderPadding = function calcCanvasRenderPadding(playerArr, aLen, rLen, tileSize) {
  let startRow = 0,
    padRow = 0,
    startCol = 0,
    padCol = 0,
  ...
  if (paddingCache['' + playerArr]) {
    ({ startRow, startCol, renderArrHeight, renderArrWidth, sX, sY } =
      paddingCache['' + playerArr]);
  } else {
  ...
  return { startRow, startCol, renderArrHeight, renderArrWidth, sX, sY };
}
...
const backgroundArray = function createBackgroundArray(arrSize) {
  ...
  const choosePaths = function newPaths(rm, rooms, stitchArr, chkRow, chkCol, tieRule, tiedRow) {
    ...
    if (rm.yMax - rm.yPadT - rm.yPadB < riskySmall &&
      !goRt &&
      [1,2,4,5].includes(tieRule)) {
